...
- 🎯 Entry points to execute your program using `python -m <fast_case>` or `$ fast_case` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/qinlu1024/fast_case/branch/main/graph/badge.svg?token=fast_case_token_here)](https://codecov.io/gh/qinlu1024/fast_case)
[![CI](https://github.com/qinlu1024/fast_case/actions/workflows/main.yml/badge.svg)](https://github.com/qinlu1024/fast_case/actions/workflows/main.yml)

Awesome fast_case created by qinlu1024

## Install

from source
```bash
git clone https://github.com/qinlu1024/fast_case fast_case
cd fast_case
make install
```
...
```bash
pip install fast_case
```

## Executing

```bash
$ fast_case run --port 8080
```

or

```bash
python -m fast_case run --port 8080
```

or

```bash
$ uvicorn fast_case:app
```
...
```bash
❯ fast_case --help
Usage: fast_case [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fast_case create-user --help
Usage: fast_case create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fast_case create-user admin admin --superuser
```
...
Run with `fast_case run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fast_case create-user`.
...
-----------------------------------------------------
fast_case/__init__.py              4      0   100%
fast_case/app.py                  16      1    94%
fast_case/cli.py                  21      0   100%
fast_case/config.py                5      0   100%
fast_case/db.py                   10      0   100%
fast_case/models/__init__.py       0      0   100%
fast_case/models/content.py       47      1    98%
fast_case/routes/__init__.py      11      0   100%
fast_case/routes/content.py       52     25    52%
fast_case/routes/security.py      15      1    93%
fast_case/routes/user.py          52     26    50%
fast_case/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from fast_case.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fast_case/default.toml.

### As environment variables
```bash
export fast_case_KEY=value
export fast_case_KEY="@int 42"
export fast_case_KEY="@jinja {{ this.db.uri }}"
export fast_case_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
fast_case_ENV=production fast_case run
```
